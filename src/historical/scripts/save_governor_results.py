...
# Path to JSON file
JSON_PATH = os.path.join(os.path.dirname(__file__), "raw_iebc_results_pdfs_2027", "2017_governor_results.json")
...
def main():
    ...
    for entry in data:
        ...
        if created:
            print(f"Created results for {county.name} - {candidate_other_names} {candidate_surname}")
        else:
            print(f"Results already exist for {county.name} - {candidate_other_names} {candidate_surname}")
