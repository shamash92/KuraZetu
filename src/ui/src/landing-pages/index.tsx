...
function AtlasBackground() {
    ...
    useEffect(() => {
        ...
        let captionTimer = 0;
        let stopped = false;
        const paths = Array.from(svg.querySelectorAll<SVGPathElement>(".kz-atlas-county"));
        ...
        paths.forEach((path, index) => {
            ...
            path.style.setProperty("--delay", `${(180 + (distance / maxDistance) * 1000).toFixed(0)}ms`);
        });
        ...
        const shuffled = data.counties.map((_, index) => index).sort(() => Math.random() - 0.5);
        let cursor = 0;
        ...
        const tween = (target: number[], duration: number) => {
            ...
            const tick = (now: number) => {
                ...
                svg.setAttribute(
                    "viewBox",
                    from.map((value, index) => value + (target[index] - value) * eased).join(" ")
                );
        ...
        const focusNext = () => {
            ...
            const aspect = rect.width && rect.height ? rect.width / rect.height : data.w / data.h;
    ...
    return (
        <div className="kz-atlas-layer" aria-hidden="true">
            <svg ref={svgRef} viewBox={`0 0 ${data.w} ${data.h}`} preserveAspectRatio="xMidYMid meet">
        ...
        <>
            ...
            <header className="kz-topbar">
                ...
                <nav className="kz-nav-links" aria-label="Main navigation">
                    ...
                    <a href="https://github.com/shamash92/KuraZetu.git" target="_blank" rel="noreferrer">
                        Contribute
                    </a>
                ...
                {open && (
                    <nav className="kz-mobile-nav" aria-label="Mobile navigation">
                        <a href="/ui/game/">pinVerify254</a>
                        <a href="https://github.com/shamash92/KuraZetu.git">Contribute</a>
        ...
        <section className="kz-stage" aria-label="KuraZetu app preview">
            <div className="kz-stage-inner">
                <div className="kz-float kz-float-constituency">
                    <div className="kz-float-icon"><ShieldCheck size={21} /></div>
                    <div><strong>Westlands</strong><span>62 / 78 stations · 6 verifiers</span></div>
                </div>
                <div className="kz-float kz-float-live"><Clock3 size={14} /> Tallying live</div>
                ...
                <div className="kz-phone">
                    <div className="kz-phone-screen">
                        ...
                        <div className="kz-status"><span>9:41</span><span>5G&nbsp;&nbsp; 82%</span></div>
                        <div className="kz-screen-body">
                            ...
                            <div className="kz-tile-grid">
                                {phoneTiles.map(([label, Icon], index) => (
                                    <div className={`kz-phone-tile kz-phone-tile-${index + 1}`} key={label}>
                            ...
                            <div className="kz-result-row">
                                <div><strong>Dagoretti N.</strong><span>41 / 68 stations · 4 verifiers</span></div>
                                <b>60%</b>
                            </div>
                        </div>
                        <div className="kz-tabbar">
                            <span className="is-active"><i><Home size={14} /></i>Home</span>
                            <span><i><List size={14} /></i>Results</span>
                            <span><i><MapPin size={14} /></i>Map</span>
                            <span><i><User size={14} /></i>Me</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="kz-float kz-float-tally">
                    ...
                    <a href="/accounts/login/">View full report <ArrowRight size={14} /></a>
                </div>
                <div className="kz-float kz-float-alert">
                    <i><AlertTriangle size={18} /></i>
                    <div><strong>Alert</strong><span>Discrepancy at Station 114B</span></div>
                </div>
                <div className="kz-float kz-float-user">
                    <i>AN</i>
                    <div><strong>Achieng N.</strong><span>Verifier · Kisumu Central</span></div>
                </div>
            </div>
        </section>
    );
        ...
        <footer className="kz-footer">
            <div className="kz-footer-inner">
                <div className="kz-foot-grid">
                    <div>
                        <Brand />
                        <p>A non-partisan, open-source citizen-tech project. Powered by Kiongozi. Built in Nairobi.</p>
                    </div>
                    <div>
                ...
                <div className="kz-foot-bottom">
                    <span>© 2026 Kura Zetu · MIT licensed · Not affiliated with IEBC</span>
                    <span>Built in Nairobi · Designed for elections, not against them.</span>
                </div>
            </div>
        </footer>
    );
    ...
    return (
        <main className="kz-landing">
            ...
            <section className="kz-hero">
                <h1>Election results,<br />uploaded by <span>you</span>.</h1>
                <p>
                    You photograph the Form 34A at your polling station. We aggregate and verify.
                    Anyone, anywhere, sees the count as it happens.
                </p>
                <div className="kz-hero-actions">
                    <a className="kz-button kz-button-ink kz-button-large" href="/accounts/login/">
                        See live results <ArrowRight size={16} />
                    </a>
                    <a className="kz-button kz-button-large" href="https://kurazetu.readthedocs.io/" target="_blank" rel="noreferrer">
                        Read the docs
                    </a>
                </div>
                <div className="kz-pill-row">
                    <a className="kz-pill kz-pill-ink" href="https://github.com/shamash92/KuraZetu.git"><Github size={14} />Contribute</a>
                    <a className="kz-pill kz-pill-mint" href="/ui/download-apk/"><Smartphone size={14} />Android</a>
                    <span className="kz-pill kz-pill-blue"><Smartphone size={14} />iOS coming soon</span>
                    <a className="kz-pill kz-pill-coral" href="/ui/game/"><MapPin size={14} />PinVerify254</a>
                </div>
            </section>
            ...
            <section className="kz-trust-strip">
                ...
                <div><b>Open source</b><b>Polling-station level</b><b>Community verified</b><b>Public API</b></div>
            </section>

            <section className="kz-compare" id="about">
                <div className="kz-section-heading kz-section-heading-centered">
                    <span>Read this first</span>
                    <h2>What KuraZetu is. And just as importantly, what it is <em>not</em>.</h2>
                    <p>We are not the IEBC, a political party, or legal authority. We are a parallel record built by citizens, for citizens.</p>
                </div>
                <div className="kz-compare-table">
                    <div className="kz-compare-head"><b>Aspect</b><b>What it is</b><b>What it is not</b></div>
                    {comparisonRows.map((row) => (
                        <div className="kz-compare-row" key={row.aspect}>
                            <strong>{row.aspect}</strong>
                            <p><i className="is-yes"><Check size={13} /></i>{row.is}</p>
                            <p><i className="is-no"><X size={13} /></i>{row.isNot}</p>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="kz-how">
                <div className="kz-section-heading">
                    ...
                    <h2>From the paper on the wall to the public dashboard, in four steps.</h2>
                </div>
            ...
            <section className="kz-why">
                <div>
                    ...
                    <h2>Every number has a <b>Form 34A</b> behind it. Every form has a <b>community verifier</b> behind it.</h2>
                </div>
            ...
            <section className="kz-cta-wrap">
                <div className="kz-cta">
                    <div>
                        ...
                        <h2>Your phone.<br />Your polling station.<br />Your verified Form 34A.</h2>
                        <p>Sign up with your phone number. No password, email, or public name required.</p>
                    </div>
                    <div>
                        <a href="/ui/download-apk/">Download for Android <ArrowRight size={16} /></a>
                        <a className="is-ghost" href="/ui/signup/">Create an account <ArrowRight size={16} /></a>
                        <a className="is-text" href="/accounts/login/">Just browse the results</a>
                    </div>
                </div>
            </section>
